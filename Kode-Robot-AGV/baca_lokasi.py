...
class DualMFRC522:
    ...
    def read_no_block(self, reader):
        ...
        return card_id, None  # Assuming there is no text data
...
def Baca_jenis_barang():
    ...
    try:
        while not jenis_barang:
            id1, text1 = reader.read_no_block(reader.reader1)
            if id1:
...
def Baca_lokasi():
    ...
    try:
        while not lokasi:
            id2, text2 = reader.read_no_block(reader.reader2)
            if id2:
...
if __name__ == "__main__":
    ...
    lokasi = Baca_lokasi()
    print(f"Jenis Barang: {jenis_barang}, Lokasi: {lokasi}")
